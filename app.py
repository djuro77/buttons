...
buttons.append({"category":"Temp", "label":"Welcome", "content":"Hello"})
...
def new_button(category=None, command=None, text=None, row=0, column=0):
    """Adds new button to the frame_buttons - displays button"""
    global next_column
    if next_column[category] > 30:
        print("Max number of Buttons")
        return
    Button(frame_buttons, text=text, command=command, width=10).grid(column=column, row=row)
...
def build_buttons():
    ...
    for b in buttons:
        new_button(category=b['category'], command=clipboard_command(b['content']), text=b['label'], row=category_row[b['category']], column=next_column[b['category']])
...
## Button actions - to be passed as Button command= kwarg  ##

def command_new_button():
    label = entry_new_label.get()
    if label in [button['label'] for button in buttons]:
    ...
    new_button(category=category, command=command, text=label, row=row, column=next_column[category])
    buttons.append({"category":category, "label":label, "content":content})
    update_delete_menu()
...
Button(frame_add_new, text="ADD", command=command_new_button).grid(column=7, row=0)

Button(frame_add_new, text="Delete =>", command=command_delete_button).grid(column=8, row=0)
...
menu_button_delete.grid(column=9, row=0)

Button(frame_add_new, text="Import", command=command_import).grid(column=10, row=0)
Button(frame_add_new, text="Export", command=pickle_out).grid(column=11, row=0)
